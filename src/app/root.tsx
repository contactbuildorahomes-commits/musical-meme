...
const useCodeGen = () => {
  // Removed dev-only codegen messaging
};
    ...
    <html lang="en">
      <head>
        {import.meta.env.NEXT_PUBLIC_GA_ID && (
          <>
            <script async src={`https://www.googletagmanager.com/gtag/js?id=${import.meta.env.NEXT_PUBLIC_GA_ID}`}></script>
            <script
              dangerouslySetInnerHTML={{
                __html: `window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${import.meta.env.NEXT_PUBLIC_GA_ID}');`,
              }}
            />
          </>
        )}
        ...
        <meta name="twitter:site" content="@briefly" />
        {import.meta.env.NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION && (
          <meta name="google-site-verification" content={import.meta.env.NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION} />
        )}
        <Meta />
        <Links />
        {(() => {
          const site = import.meta.env.NEXT_PUBLIC_SITE_URL || '';
          const canonical = site && pathname ? new URL(pathname, site).toString() : undefined;
